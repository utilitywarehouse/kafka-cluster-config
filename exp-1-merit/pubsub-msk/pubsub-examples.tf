...
module "example_producer" {
  ...
  produce_topics   = [kafka_topic.pubsub_examples.name, kafka_topic.pubsub_examples2.name]
  cert_common_name = "pubsub/example-producer"
}

module "example_process_individually_consumer" {
  source           = "../../modules/tls-app"
  consume_topics   = [kafka_topic.pubsub_examples.name, kafka_topic.pubsub_examples2.name]

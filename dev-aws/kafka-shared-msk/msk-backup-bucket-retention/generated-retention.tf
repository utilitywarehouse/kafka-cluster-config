...
resource "aws_s3_bucket_lifecycle_configuration" "msk_topics_retention" {
  ...
  rule {
    ...
    status = "Enabled"
    expiration { days = 6 }
    filter { prefix = "msk-backup-parquet/pubsub.restore-test.data-infra.uw.data-infra.pubsubbrige.snowplow/" }
  }
  ...
  rule {
    ...
    status = "Enabled"
    expiration { days = 92 }
    filter { prefix = "msk-backup-parquet/pubsub.restore-test.unicom.status/" }
  }

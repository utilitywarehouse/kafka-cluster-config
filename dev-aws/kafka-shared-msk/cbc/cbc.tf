...
module "cbc_fraud_detection_consumer" {
  ...
  produce_topics   = [kafka_topic.FraudEvents.name, kafka_topic.lifecycle_events_v2.name]
...
module "cbc_events_indexer" {
  source           = "../../../modules/tls-app"
  consume_topics   = [kafka_topic.FraudEvents.name, kafka_topic.rating_events_v3.name, kafka_topic.lifecycle_events_v2.name, kafka_topic.charges_events_v1.name, kafka_topic.challenge_events_v1.name]
...
module "cbc_network_projector" {
  source           = "../../../modules/tls-app"
  consume_topics   = [kafka_topic.rating_events_v3.name]
...
module "cbc_push_notifications_consumer" {
  ...
  cert_common_name = "cbc/cbc-push-notifications"
}

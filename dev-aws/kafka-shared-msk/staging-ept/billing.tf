...
resource "kafka_topic" "fixed_width_file_processing_events" {
  name               = "staging-ept.fixed-width-file-processing-events"
...
resource "kafka_topic" "bill_reconciliation_error_events" {
  name               = "staging-ept.bill-reconciliation-error-events"
  replication_factor = 3
  partitions         = 2
  config = {
    # store data zstd compressed
...
resource "kafka_topic" "unified_bill_ready_events" {
  name               = "staging-ept.unified-bill-ready-events"
  replication_factor = 3
  partitions         = 2
  config = {
    # store data zstd compressed
...
resource "kafka_topic" "billing_engine_events_bce_deadletter" {
  name               = "staging-ept.billing-engine-events-bce-deadletter"
  replication_factor = 3
  partitions         = 2
  config = {
    # store data zstd compressed

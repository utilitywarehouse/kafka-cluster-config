...
resource "kafka_topic" "otlp_spans_temp" {
  name               = "otel.otlp_spans_temp"
  replication_factor = 3
  partitions         = 10
  config = {
...
module "otel_collector" {
  ...
  produce_topics   = [kafka_topic.otlp_spans.name, kafka_topic.otlp_spans_temp.name]
...
module "tempo_distributor" {
  ...
  consume_topics = [
    ...
    kafka_topic.otlp_sampled_spans_temp.name,
  ]
...
resource "kafka_topic" "otlp_sampled_spans_temp" {
  name               = "otel.otlp_sampled_spans_temp"
  replication_factor = 3
  partitions         = 10
  config = {
...
module "otel_tail_sampling_collector" {
  source           = "../../modules/tls-app"
  consume_topics   = [kafka_topic.otlp_spans.name, kafka_topic.otlp_spans_temp.name]
  produce_topics   = [kafka_topic.otlp_sampled_spans.name, kafka_topic.otlp_sampled_spans_temp.name]
